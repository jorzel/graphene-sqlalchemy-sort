...
if MYPY:
    ...
    from sqlalchemy.orm.query import _MapperEntity  # noqa: F401; pragma: no cover
...
class FilterSet(graphene.InputObjectType):
    ...
    DESCRIPTIONS = {
        ...
        OVERLAP: ("Array has elements in common with an argument array expression."),
    ...
    @classmethod
    def _register_extra(cls, extra_expressions: dict, extra_allowed_filters: dict):
        """
    ...
    def aliased(
        ...
        if isinstance(query, Query):
            ...
            warnings.warn(
                "Graphene resolve info is deprecated, use SQLAlchemy query. " + example,
                DeprecationWarning,
                stacklevel=2,
            )
    ...
    def _aliases_from_query(cls, query: Query) -> "Dict[str, _MapperEntity]":
        ...
        if join_entities:
            aliases = {
                (mapper._target, mapper.name): mapper.entity for mapper in join_entities
            }
        else:
            aliases = {
    ...
    def _generate_filter_fields(
        ...
        for op in expressions:
            ...
            doc = cls.DESCRIPTIONS.get(op)
            try:
                filter_field = cls.FILTER_OBJECT_TYPES[op](field_type, nullable, doc)
            except KeyError:
    ...
    @classmethod
    def filter(cls, info: ResolveInfo, query: Query, filters: "FilterType") -> Query:
        """
    ...
    def _translate_many_filter(
        ...
        if isinstance(filters, list):
            for f in filters:
                query, local_filters = cls._translate_many_filter(info, query, f, and_)
...
class SchemaQuery(graphene.ObjectType):
    ...
    def resolve_examples(self, info, **kwargs):
        ...
        if kwargs.get("filters"):
            query = ExampleFilter().filter(info, query, kwargs["filters"])
        if kwargs.get("sort"):
